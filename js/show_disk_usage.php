<?php

// 默认展示近12个小时的曲线图
$interval_time = isset($interval_time) ? $interval_time : 'DATE_SUB(now(),interval 12 hour)'; 

require 'conn.php';
$disk_info_sql="SELECT mount FROM os_disk_history WHERE HOST='{$host}' AND tag='{$tag}' GROUP BY mount LIMIT 10";
$disk_result = mysqli_query($conn,$disk_info_sql);

while( list($disk_mount) = mysqli_fetch_array($disk_result) ){
?>

    <div id="<?php echo $disk_mount;?>" style="height:400px"></div>
    <script type="text/javascript">
              var  myChart = echarts.init(document.getElementById('<?php echo $disk_mount;?>'));
              var arr1=[],arr2=[];
              function arrTest(){
                $.ajax({
                  type:"post",
                  async:false,
		  url:"get_data/db_disk_usage_graph_getdata.php?fn=index&host=<?php echo $host;?>&tag=<?php echo $tag;?>&mount=<?php echo $disk_mount;?>
		       &interval_time=<?php echo $interval_time;?>",
                  data:{},
                  dataType:"json",
                  success:function(result){
                    if (result) {
                      for (var i = 0; i < result.length; i++) {
                          arr1.push(result[i].create_time);
			  arr2.push(result[i].disk_usage);
                          //arr3.push(result[i].count);
                      }
                    }
                  }
                })
                return arr1,arr2;
              }
              arrTest();

              var  option = {
		    title: {
			text: 'disk_usage使用率图表',
		        //backgroundColor: 'FFFFFF'
		    },
                    tooltip: {
			trigger: 'axis'
                        //show: true
                    },
                    legend: {
                       data:['<?php echo $disk_mount;?>分区使用率统计']
                    },
                    xAxis : [
                        {
                            type : 'category',
                            data : arr1
                        }
                    ],
                    yAxis : [
                        {
                            type : 'value',
			    axisLabel: {
				formatter:'{value}%'
                            }
			}
                    ],

                    dataZoom: [
                          {   // 这个dataZoom组件，默认控制x轴。
                                //type: 'inside',
                                type: 'slider', // 这个 dataZoom 组件是 slider 型 dataZoom 组件
                                //inverse: true,
                                start: 100,      // 左边在 10% 的位置。
                                end: 80         // 右边在 60% 的位置。
                           }
                    ],

                    grid:{
                            x2: 60 ,
                            bottom: "70px"
                    },

                    series : [
                        {
                            "name":"<?php echo $disk_mount;?>分区使用率统计",
                            //"type":"bar",
			    "type":"line",
			    "smooth": "true",
                            "data":arr2,
			    stack: '秒',
			    areaStyle: {
				normal: {
				 "color": '#FBD0E5' //改变区域颜色
				}	
			    },
			    itemStyle : {
			    normal : { 
			        color:'#8cd5c2',
			        lineStyle: {
					color: '#3300FF',
					width: 3,
			    }}}
                        }
                    ]
                };
                // 为echarts对象加载数据
                myChart.setOption(option);
            // }
    </script>
	
<br><hr><br>
	
<?php
}
?>	
